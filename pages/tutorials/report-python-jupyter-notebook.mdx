...
In this tutorial we are going to create a simple financial report for stock companies. The report will be generated based on ticker and selected time period.
...
- `mercury` will be used for interactive widgets and for running notebook as web app,
- `pandas` is for data manipulation,
- `yfinance` will be used for downloading financial historical data,
...
<Callout type="info">
Please notice that during development of the notebook nothing is happening after widget change. You can interact with widget but to see change in other cells you need to manually execute them,

When notebook is served with `mercury` then cells are automatically re-executed after widget update. Only cells below updated widget are re-executed.
</Callout>
...
Please notice, that we do a candle plot for short periods (less than 1 year) and line plot for longer periods.

## Display Data Frame 

Call `tail(10)` on our Data Frame to display latest 10 samples of data:
...
In the same directory as notebook file, please start `mercury` server with the below command:

```
mercury run
```

It will start local server and publish all notebooks from the current directory as web applications. Please open `http://127.0.0.1:8000` and check your app.

You can change ticker value and control history with slider:
